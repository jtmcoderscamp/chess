...
export default class BoardPosition {
    constructor(positionArray) {
        if (positionArray) {
            this.position = BoardPosition.clonePositionArray(positionArray);
        } else {
    ...
    static clonePositionArray(board) {
        let clonedBoard = [];
        for (let x = 0; x < 8; x++) {
            clonedBoard[x] = [];
            for (let y = 0; y < 8; y++) {
                if (board[x][y] === null || board[x][y] instanceof ChessPiece) {
                    clonedBoard[x][y] = board[x][y];
                } else throw new TypeError("Attempting to clone invalid position array");
            }
        }
        return board;
    }
